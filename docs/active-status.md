...
- **Wave 4 token-passthrough** 🟡 **partially shipped (commits pending Oracle APPROVE archive, 2026-09-09)** — Wave 1 #1 GAP 同源, 修 stop_token 透传链路 (execute_node/execute_yield + process/react_once + run/ipc_loop_and_wait); 3 changes:
  - `fix-yield-node-token-passthrough` ✅ **fix commit 9795784 + c2806af (Oracle SHIP-with-fixes P1+P2 applied) + scaffold 60f3378**; Oracle 复核 bg_d15ba865 pending
  - `fix-orchestrator-token-passthrough` ✅ **fix commit 4598fda + scaffold 747a2ed**; Oracle 复核 bg_bf1fd293 pending
  - `fix-skill-interpreter-token-and-timeout` ✅ **fix commit bafce05 + scaffold b47ba0c**; Oracle 复核 bg_c757d391 pending
  - **3 Oracle 后台并发复核** + 4 new TEST_CASEs (test_yield_node ×2, test_simple_orchestrator ×1, test_skill_interpreter ×1) + 3 pre-existing test expectation 修正 (7.2/7.8/7.19); ctest 228/228 PASS 零回归; adr_lint 68 ADR + docs_drift 0 DRIFT; archive 待 3 Oracle 全部 APPROVE 后逐个 archive + active-status ctest 计数更新
