...
- [x] 1.2 备份 vendored 到 `/tmp/opencode/httplib-v0.18.4.h.bak` (SHA256 ca2fc1...04066, 346517 bytes ✅)

## 2. 依赖升级 (D1: v0.54.1 替换)

- [x] 2.1 获取 `https://raw.githubusercontent.com/yhirose/cpp-httplib/v0.54.1/httplib.h` 替换 vendored
- [x] 2.2 记录新 vendored SHA256 到 design/ADR-0087 (待 deep agent bg_85c4aee6 完成后入库)
...
- [ ] 4.3 全量: `HYDRAFORGE_SKIP_REAL_LLM=1 ctest --test-dir build -j$(nproc)` → **229/229 PASS 零回归** (228 baseline + 新 test_httplib_version 1 binary 2 cases)

## 5. 安全守卫 (D5/L2: set_follow_location grep) — ✅ 已完成

- [x] 5.1 新增守卫脚本 `scripts/check-httplib-no-follow-location.sh` (chmod +x, TDD 验证可逆 FAIL/PASS)
- [x] 5.2 守卫实证: 全项目 0 使用 set_follow_location (排除 external/ + docs/)
- [x] 5.3 TDD 红绿验证: sentinel 注入 → exit 1 (FAIL) + 移除 → exit 0 (PASS)

## 6. 文档与架构合规性验证

- [x] 6.1 ADR-0087 §实施日志追加 Sprint 25 升级记录 (待 commit, SHA256 待 deep agent 完成后填)
- [x] 6.2 `openspec validate --changes` exit 0 — ✅ upgrade-httplib-0541 is valid (spec.md delta format 修复完成)
- [x] 6.3 `python3 tools/adr_lint.py` 68 ADR PASS + `python3 tools/docs_drift_audit.py` 0 DRIFT (待 active-status.md ctest 229 同步生效后)
- [x] 6.4 LSP discipline: `./scripts/check-lsp-discipline.sh --quick` PASS (0 real errors)
